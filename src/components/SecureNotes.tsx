...
import { FileText, Plus, Eye, EyeOff, Trash2, Edit3, Save, X, Lock } from 'lucide-react';
...
export function SecureNotes() {
  ...
  return (
    <div className="h-full flex animate-fade-in">
      {/* List panel */}
      <div className="flex flex-col border-r" style={{ width: '280px', flexShrink: 0, borderColor: 'var(--c-border-m)' }}>
            ...
            <div className="space-y-1">
              {notes.map(note => (
                <div key={note.id} onClick={() => setSelectedId(note.id)}
                  className={`px-3 py-3 rounded-lg cursor-pointer transition-all border ${
                    selectedId === note.id ? 'border-vault-gold border-opacity-20' : 'border-transparent'
                  }`}
                  style={{
      ...
      {/* Detail panel */}
        <div className="flex-1 overflow-hidden flex flex-col" style={{ background: 'var(--c-active-bg)' }}>
        {/* Add new note form */}
        {showAdd && (
          <div className="p-6 border-b animate-slide-up" style={{ borderColor: 'var(--c-border-m)' }}>
            <div className="max-w-2xl space-y-4">
              <div className="flex items-center justify-between">
                <h3 className="font-semibold" style={{ color: 'var(--c-text)' }}>New Secure Note</h3>
                <button onClick={() => setShowAdd(false)} style={{ color: 'var(--c-text-m)' }}><X size={16} /></button>
              </div>
                    ...
                    {showContent[selected.id] ? (
                      ...
                      <div className="flex flex-col items-center py-8 gap-3">
                        ...
                        <p className="text-sm" style={{ color: 'var(--c-text-f)' }}>
                          Click <strong style={{ color: 'var(--c-text-s)' }}>Show</strong> to reveal content
                        </p>
